...
def main():
	...
	# judge the binary is i64, else do nothing
	analyseFlag = BugBinaryPath[-4:]=='.i64' and TargetBinaryPath[-4:]=='.i64'

	if analyseFlag:

		# deal the bug binary with ida
		# delete .id0, .id1, .nam, .til
		...
		#print "len(BugFunctionList)",len(BugFunctionList)




		if len(BugFunctionList) != 0:

			# deal the target binary with ida
			# delete .id0, .id1, .nam, .til
			...
			#print "len(TargetFunctionList)",len(TargetFunctionList)
